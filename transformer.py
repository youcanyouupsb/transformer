...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, dropout=0.1, max_len=5000):
        ...
        self.pos_table = torch.FloatTensor(pos_table).to('cpu')        # enc_inputs: [seq_len, d_model]

    def forward(self, enc_inputs):                                  # enc_inputs: [batch_size, seq_len, d_model]
        enc_inputs += self.pos_table[:enc_inputs.size(1), :]
        return self.dropout(enc_inputs.to('cpu'))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):    # input_Q: [batch_size, len_q, d_model]
        ...
        return nn.LayerNorm(d_model).to('cpu')(output + residual), attn
...
class PoswiseFeedForwardNet(nn.Module):
    ...
    def forward(self, inputs):                                  # inputs: [batch_size, seq_len, d_model]
        ...
        return nn.LayerNorm(d_model).to('cpu')(output + residual)  # [batch_size, seq_len, d_model]
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):                         # dec_inputs: [batch_size, tgt_len]
        ...
        dec_outputs = self.pos_emb(dec_outputs).to('cpu')                              # [batch_size, tgt_len, d_model]
        dec_self_attn_pad_mask = get_attn_pad_mask(dec_inputs, dec_inputs).to('cpu')   # [batch_size, tgt_len, tgt_len]
        dec_self_attn_subsequence_mask = get_attn_subsequence_mask(dec_inputs).to('cpu')  # [batch_size, tgt_len, tgt_len]
        dec_self_attn_mask = torch.gt((dec_self_attn_pad_mask +
                                       dec_self_attn_subsequence_mask), 0).to('cpu')   # [batch_size, tgt_len, tgt_len]
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.projection = nn.Linear(d_model, tgt_vocab_size, bias=False).to('cpu')
